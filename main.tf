...
resource "oci_devops_build_pipeline_stage" "this" {
  for_each = local.build_stage_map
  ...
  build_pipeline_stage_predecessor_collection {
    dynamic "items" {
      for_each = length(each.value.predecessor_keys) == 0 ? ["__PIPELINE__"] : each.value.predecessor_keys
      content {
        id = items.value == "__PIPELINE__" ? oci_devops_build_pipeline.this[each.value.pipeline_key].id : oci_devops_build_pipeline_stage.this["${each.value.pipeline_key}:${items.value}"].id
      }
    }
  }
...
resource "oci_devops_deploy_stage" "this" {
  for_each = local.deploy_stage_map
  ...
  oke_canary_deploy_stage_id               = each.value.stage_type == "OKE_CANARY_TRAFFIC_SHIFT" ? oci_devops_deploy_stage.this["${each.value.pipeline_key}:${each.value.oke_canary_deploy_stage_key}"].id : null
  oke_canary_traffic_shift_deploy_stage_id = each.value.stage_type == "OKE_CANARY_APPROVAL" ? oci_devops_deploy_stage.this["${each.value.pipeline_key}:${each.value.oke_canary_traffic_shift_stage_key}"].id : null
  oke_blue_green_deploy_stage_id           = each.value.stage_type == "OKE_BLUE_GREEN_TRAFFIC_SHIFT" ? oci_devops_deploy_stage.this["${each.value.pipeline_key}:${each.value.oke_blue_green_deploy_stage_key}"].id : null
  ...
  deploy_stage_predecessor_collection {
    dynamic "items" {
      for_each = length(each.value.predecessor_keys) == 0 ? ["__PIPELINE__"] : each.value.predecessor_keys
      content {
        id = items.value == "__PIPELINE__" ? oci_devops_deploy_pipeline.this[each.value.pipeline_key].id : oci_devops_deploy_stage.this["${each.value.pipeline_key}:${items.value}"].id
      }
    }
  }
